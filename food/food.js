$("searchresult")[0].style.height =
  `${$("searchresult")[0].children.length * 200}px`;
...
function isaacSearch(key) {
  if (key[0] == " ") {
    // check if white space before search
    key = key.trimStart();
  }
  if (targetTags.length > 0) {
    ...
    for (let i = 0; i < $("article.filtered").length; i++) {
      if (
        !$("article.filtered")
          [i].children[0].children[0].children[0].textContent.toLowerCase()
    ...
    for (let i = 0; i < $("if-title").length; i++) {
      if (
        !$("if-title")[i].textContent.toLowerCase().includes(key.toLowerCase())
      ) {
    ...
    for (let i = 0; i < $("article").length; i++) {
      for (let tag = 0; tag < foodBlogMetaData.get(i).length; tag++) {
        if (targetTags.indexOf(foodBlogMetaData.get(i)[tag]) != -1) {
          if (
            $("article")[i].classList.contains("out") &&
            $("if-title")
              [i].textContent.toLowerCase()
              .includes($("#food-search")[0].value.toLowerCase())
          ) {
            $("article")[i].classList.remove("out");
          }
